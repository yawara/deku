...
fn emit_struct(input: &DekuData) -> Result<TokenStream, syn::Error> {
    ...
    if input.ctx.is_none() || (input.ctx.is_some() && input.ctx_default.is_some()) {
        let to_bits_body = wrap_default_ctx(
            quote! {
                match *self {
                    #destructured => {
                        let mut __deku_acc: ::#crate_::bitvec::BitVec<::#crate_::bitvec::Lsb0, u8> = ::#crate_::bitvec::BitVec::new();
        ...
        tokens.extend(quote! {
            impl #imp core::convert::TryFrom<#ident> for ::#crate_::bitvec::BitVec<::#crate_::bitvec::Lsb0, u8> #wher {
            ...
            impl #imp DekuContainerWrite for #ident #wher {
                fn to_bytes(&self) -> Result<Vec<u8>, ::#crate_::DekuError> {
                    let mut acc: ::#crate_::bitvec::BitVec<::#crate_::bitvec::Lsb0, u8> = self.to_bits()?;
                    Ok(acc.into_vec())
                }

                #[allow(unused_variables)]
                fn to_bits(&self) -> Result<::#crate_::bitvec::BitVec<::#crate_::bitvec::Lsb0, u8>, ::#crate_::DekuError> {
                    #to_bits_body
                }
            }
        });
    ...
    tokens.extend(quote! {
        ...
        impl #imp DekuWrite<#ctx_types> for #ident #wher {
            #[allow(unused_variables)]
            fn write(&self, __deku_output: &mut ::#crate_::bitvec::BitVec<::#crate_::bitvec::Lsb0, u8>, #ctx_arg) -> Result<(), ::#crate_::DekuError> {
                #write_body
            }
        }
    });
    ...
    if input.ctx.is_some() && input.ctx_default.is_some() {
        ...
        tokens.extend(quote! {
            impl #imp DekuWrite for #ident #wher {
                #[allow(unused_variables)]
                fn write(&self, __deku_output: &mut ::#crate_::bitvec::BitVec<::#crate_::bitvec::Lsb0, u8>, _: ()) -> Result<(), ::#crate_::DekuError> {
                    #write_body
                }
            }
        });
