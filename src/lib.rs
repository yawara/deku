...
- `deku::input_bits: &BitSlice<Lsb0, u8>` - The initial input in bits
- `deku::rest: &BitSlice<Lsb0, u8>` - Remaining bits to read
- `deku::output: &mut BitSlice<Lsb0, u8>` - The output bit stream
...
pub trait DekuRead<'a, Ctx = ()> {
    ...
    fn read(
        input: &'a bitvec::BitSlice<bitvec::Lsb0, u8>,
        ctx: Ctx,
    ) -> Result<(&'a bitvec::BitSlice<bitvec::Lsb0, u8>, Self), DekuError>
    where
        Self: Sized;
}
...
pub trait DekuWrite<Ctx = ()> {
    ...
    fn write(
        &self,
        output: &mut bitvec::BitVec<bitvec::Lsb0, u8>,
        ctx: Ctx,
    ) -> Result<(), DekuError>;
}
...
pub trait DekuContainerWrite: DekuWrite<()> {
    ...
    fn to_bits(&self) -> Result<bitvec::BitVec<bitvec::Lsb0, u8>, DekuError>;
}
